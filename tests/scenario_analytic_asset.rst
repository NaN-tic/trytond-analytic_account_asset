...
Create an asset::
    ...
    >>> asset_template.analytic_accounts = analytic_selection
    >>> asset_template.depreciation_duration = Decimal(24)
    >>> asset_template.save()
    >>> asset_analytic_account, = asset_template.analytic_accounts_used
    >>> asset_analytic_account == deprecation_analytic_account
    True
...
Create Moves for 3 months::
    ...
    True
    >>> depreciation_debit = sum([a.debit for a in AnalyticLine.find([(
    ...     'account', '=', deprecation_analytic_account.id)])])
    >>> depreciation_debit == Decimal('0.0')
    True
    >>> depreciation_credit = sum([a.credit for a in AnalyticLine.find([(
    ...     'account', '=', deprecation_analytic_account.id)])])
    >>> depreciation_credit == Decimal('112.5')
    True
...
Update the asset::
    ...
    >>> depreciation_debit = sum([a.debit for a in AnalyticLine.find([(
    ...     'account', '=', deprecation_analytic_account.id)])])
    >>> depreciation_debit == Decimal('100.0')
    True
    >>> depreciation_credit = sum([a.credit for a in AnalyticLine.find([(
    ...     'account', '=', deprecation_analytic_account.id)])])
    >>> depreciation_credit == Decimal('112.5')
